...
class RecipeProcessingJob < ApplicationJob
  ...
  def perform(ingredients)
    ...
    recipe_img = html_doc.at_css("#recipe-media-viewer-thumbnail-container-0")&.attribute('src')&.value

    CartChannel.broadcast_to('cart', { recipe_name: recipe_name, recipe_spec: recipe_spec, recipe_ingredients: recipe_ingredients, recipe_steps: recipe_steps, recipe_img: recipe_img })
  end
end
